...
public class FlowLimitFilter extends HttpFilter {
    ...
    private void writeBlockMessage(HttpServletResponse response) throws IOException {
        ...
        response.getWriter().write(RestBean.forbidden("请求过于频繁,请稍后重试").asJsonString());
    }
    ...
    private boolean limitPeriodCheck(String ip) {
        if (Boolean.TRUE.equals(stringRedisTemplate.hasKey(Const.FLOW_LIMIT_COUNTER + ip))) {
            ...
            if (increment > 10) {
                stringRedisTemplate.opsForValue().set(Const.FLOW_LIMIT_BLOCK + ip, "1", 30, TimeUnit.SECONDS);
                return false;
            }
        } else {
